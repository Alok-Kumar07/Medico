...
// --- Main Component ---
export default function DoctorPatientProfile({ route }) {
...
            setLoading(false);
        });
        const medsSub = firestore().collection('users').doc(patientId).collection('medicines').onSnapshot(snap => setAllMeds(snap.docs.map(doc => ({ docId: doc.id, ...doc.data() }))));
...
                const doc = await firestore().collection('users').doc(patient.approvedCaregivers[0]).get();
                setCaretakerName(doc.exists ? doc.data().name || "Unnamed Caregiver" : "Caregiver not found");
            } else { setCaretakerName("Not Assigned"); }
        };
        fetchProviders();
    }, [patient]);

    // --- UPDATED HANDLER TO DISPLAY ALERT ---
    const handleRemovePatient = () => {
        Alert.alert(
            "Removal Blocked 🛑",
            `Only ${patient?.name} (the patient) has the authority to remove you as their approved doctor.`,
            [
                { 
                    text: "OK", 
                    style: "cancel" 
...
            (med.times || []).forEach(time => baseSchedule.push({ id: `${med.docId}_${time}`, docId: med.docId, name: med.name, scheduledTime: time, status: 'Pending', takenAt: null, confirmationPhotoUrl: null }));
        });
...
        const uniqueMeds = allMeds.reduce((acc, current) => { if (!acc.find(item => item.docId === current.docId)) acc.push(current); return acc; }, []);
        return uniqueMeds.map(m => ({ ...m, daysLeft: (m.times?.length > 0) ? Math.floor((m.stock || 0) / m.times.length) : 0, dailyUsage: m.times?.length || 0 }));
    }, [allMeds]);
...
    const handleConfirmMonthYear = () => { setDisplayDate(pickerDate); setSelectedDate(new Date(pickerDate.getFullYear(), pickerDate.getMonth(), 1)); setShowMonthPicker(false); };
    const renderCalendarDay = ({ item }) => {
        const isToday = item.toDateString() === today.toDateString();
        const isSelected = item.toDateString() === selectedDate.toDateString();
        return (<TouchableOpacity onPress={() => setSelectedDate(item)} style={[styles.dayPill, isSelected && styles.dayPillSelected, isToday && styles.dayPillToday]}><Text style={[styles.dayName, isSelected && styles.dayNameActive]}>{dayShort(item)}</Text><View style={[styles.dayNumWrap, isSelected && styles.dayNumWrapActive]}><Text style={[styles.dayNum, isSelected && styles.dayNumActive]}>{monthDay(item)}</Text></View></TouchableOpacity>);
    };

    if (loading || !patient) { return <SafeAreaView style={styles.safe}><ActivityIndicator size="large" color="#2563eb" /></SafeAreaView>; }
...
                        
                        {/* Grouping the functional buttons */}
                        <View style={styles.actionButtonsContainer}>
                            <TouchableOpacity style={styles.manageBtn} onPress={() => navigation.navigate('DoctorEditSchedule', { patientId: patient.id })}>
                                <Icon name="list" size={16} color="#312e81" />
                                <Text style={styles.manageBtnText}>Schedule</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={styles.manageBtn} onPress={() => navigation.navigate('DoctorAddReportScreen', { patientId: patient.id, patientName: patient.name })}>
                                <Icon name="file-text" size={16} color="#312e81" />
                                <Text style={styles.manageBtnText}>Reports</Text>
                            </TouchableOpacity>
                        </View>

                        {/* --- 2. REMOVE PATIENT BUTTON with new logic --- */}
...
                    </View>
                    <View style={styles.section}><Text style={styles.sectionTitle}>Assigned Doctor</Text><Text style={styles.sectionValue}>Dr. {doctorName}</Text><Text style={[styles.sectionTitle, { marginTop: 12 }]}>Caretaker</Text><Text style={styles.sectionValue}>{caretakerName}</Text></View>
                    <View style={styles.calendarWrap}><View style={styles.calendarHeader}><Text style={styles.sectionTitle}>Adherence Calendar</Text><TouchableOpacity onPress={() => { setPickerDate(displayDate); setShowMonthPicker(true); }} style={styles.datePickerButton}><Text style={styles.sectionTitle}>{`${displayDate.toLocaleString('default', { month: 'long' })} ${displayDate.getFullYear()}`}</Text><Text style={styles.datePickerIcon}> ▼</Text></TouchableOpacity></View><FlatList ref={calendarListRef} data={calendarDays} renderItem={renderCalendarDay} horizontal showsHorizontalScrollIndicator={false} keyExtractor={(item) => item.toISOString()} getItemLayout={(data, index) => ({ length: 80, offset: 80 * index, index })} /></View>
                    <View style={styles.section}><Text style={styles.sectionTitle}>Medicine Log</Text>{medsForSelectedDate.length > 0 ? medsForSelectedDate.map(item => { const statusColor = item.status === 'Taken' ? '#10b981' : item.status === 'Missed' ? '#ef4444' : '#f59e0b'; return (<TouchableOpacity key={item.id} style={[styles.medicineCard, { borderLeftColor: statusColor }]} onPress={() => setLogDetailModal({ visible: true, data: item })}><View style={{ flex: 1 }}><Text style={styles.medicineName}>{item.name}</Text><Text style={styles.medicineSchedule}>Scheduled: {item.scheduledTime}</Text></View><Text style={[styles.medicineStatus, { color: statusColor }]}>{item.status}</Text></TouchableOpacity>)}) : <Text style={styles.emptyText}>No medicines scheduled for this date.</Text>}</View>
                    <View style={styles.section}><Text style={styles.sectionTitle}>Medicine Stocks</Text>{medsWithStock.length > 0 ? (medsWithStock.map(item => { let stockStyle = styles.stockRowGreen; if (item.daysLeft < 4) stockStyle = styles.stockRowRed; else if (item.daysLeft < 8) stockStyle = styles.stockRowYellow; return (<View key={item.docId} style={[styles.stockRow, stockStyle]}><View style={{ flex: 2 }}><Text style={styles.cellTitle}>{item.name}</Text><Text style={styles.cellSub}>Stock: {item.stock} • Daily: {item.dailyUsage}</Text></View><View style={{ flex: 1, alignItems: 'flex-end' }}><Text style={{ fontWeight: '700', color: '#111' }}>{item.daysLeft} days left</Text></View></View>); })) : (<Text style={styles.emptyText}>No medicines found for this patient.</Text>)}</View>
                </View>
            </ScrollView>
            <Modal visible={logDetailModal.visible} transparent animationType="fade" onRequestClose={() => setLogDetailModal({ visible: false, data: null })}><Pressable style={styles.modalOverlay} onPress={() => setLogDetailModal({ visible: false, data: null })}><Pressable style={styles.modalCard}>{logDetailModal.data && (<><Text style={styles.modalTitle}>{logDetailModal.data.name}</Text><View style={styles.modalDetailRow}><Text style={styles.modalDetailLabel}>Status:</Text><Text style={[styles.modalDetailValue, { color: logDetailModal.data.status === 'Taken' ? '#16a34a' : '#ef4444' }]}>{logDetailModal.data.status}</Text></View><View style={styles.modalDetailRow}><Text style={styles.modalDetailLabel}>Taken At:</Text><Text style={styles.modalDetailValue}>{logDetailModal.data.takenAt ? fmtTime(logDetailModal.data.takenAt) : 'N/A'}</Text></View><Text style={[styles.modalDetailLabel, { marginTop: 16, marginBottom: 8 }]}>Confirmation Photo:</Text>{logDetailModal.data.confirmationPhotoUrl ? <Image source={{ uri: logDetailModal.data.confirmationPhotoUrl }} style={styles.confirmationImage} /> : <View style={styles.noPhotoContainer}><Text style={styles.noPhotoText}>No photo was uploaded.</Text></View>}<TouchableOpacity style={styles.closeButton} onPress={() => setLogDetailModal({ visible: false, data: null })}><Text style={styles.closeButtonText}>Close</Text></TouchableOpacity></>)}</Pressable></Pressable></Modal>
            <Modal visible={showMonthPicker} transparent animationType="fade"><Pressable style={styles.modalOverlay} onPress={() => setShowMonthPicker(false)}><Pressable style={styles.pickerModalCard}><Text style={styles.modalTitle}>Select Month & Year</Text><View style={styles.pickerContainer}><FlatList data={YEARS} keyExtractor={item => item.toString()} renderItem={({ item }) => (<TouchableOpacity style={[styles.pickerItem, pickerDate.getFullYear() === item && styles.pickerItemSelected]} onPress={() => setPickerDate(new Date(item, pickerDate.getMonth(), 1))}><Text style={[styles.pickerItemText, pickerDate.getFullYear() === item && styles.pickerItemTextSelected]}>{item}</Text></TouchableOpacity>)} showsVerticalScrollIndicator={false} style={styles.pickerList} initialScrollIndex={YEARS.indexOf(pickerDate.getFullYear())} getItemLayout={(data, index) => ({ length: 45, offset: 45 * index, index })} /><FlatList data={MONTHS} keyExtractor={item => item} renderItem={({ item, index }) => (<TouchableOpacity style={[styles.pickerItem, pickerDate.getMonth() === index && styles.pickerItemSelected]} onPress={() => setPickerDate(new Date(pickerDate.getFullYear(), index, 1))}><Text style={[styles.pickerItemText, pickerDate.getMonth() === index && styles.pickerItemTextSelected]}>{item}</Text></TouchableOpacity>)} showsVerticalScrollIndicator={false} style={styles.pickerList} initialScrollIndex={pickerDate.getMonth()} getItemLayout={(data, index) => ({ length: 45, offset: 45 * index, index })} /></View><View style={styles.pickerActions}><Pressable style={[styles.modalBtn, { flex: 1, marginRight: 8 }]} onPress={() => setShowMonthPicker(false)}><Text style={[styles.modalBtnText, { color: '#111' }]}>Cancel</Text></Pressable><Pressable style={[styles.modalBtn, { flex: 1, marginLeft: 8, backgroundColor: '#2563eb' }]} onPress={handleConfirmMonthYear}><Text style={[styles.modalBtnText, { color: '#fff' }]}>Confirm</Text></Pressable></View></Pressable></Pressable></Modal>
        </SafeAreaView>
    );
}
...
    // --- 3. STYLES FOR THE REMOVE BUTTON (No change needed here for the logic) ---
    removeBtn: {
        marginTop: 12,
        paddingVertical: 6,
